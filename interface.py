...
with col1:
	...
	revenu_mensuel = st.number_input("Revenu mensuel (MAD)", min_value=1000, max_value=100000, value=12000, step=500)
	montant_credit_demande = st.number_input(
		"Montant du crédit demandé (MAD)", min_value=5000, max_value=500000, value=50000, step=5000
	)
...
with col2:
	duree_remboursement_mois = st.selectbox(
		...
		format_func=lambda x: f"{x} mois ({x // 12} an)" if x >= 12 else f"{x} mois",
		index=2,
	)
	nb_credits_anterieurs = st.number_input("Nombre de crédits antérieurs", min_value=0, max_value=10, value=0, step=1)
	situation_familiale = st.selectbox(
		"Situation familiale",
		options=["marie", "celibataire", "divorce"],
		format_func=lambda x: {"marie": "Marié(e)", "celibataire": "Célibataire", "divorce": "Divorcé(e)"}[x],
	)
	type_emploi = st.radio(
		"Type d'emploi",
		options=["fonctionnaire", "salarie_prive", "independant", "sans_emploi"],
...
if st.button("� Soumettre ma demande", type="primary", use_container_width=True):
	...
	try:
		...
		if reponse.status_code == 200:
			...
			if decision == "ACCORDÉ":
				st.markdown(
					f"""<div style="background-color:#d4edda; border:2px solid #28a745; border-radius:12px; padding:24px; text-align:center;">
<h2 style="color:#155724;">✅ CRÉDIT ACCORDÉ</h2>
<p style="color:#155724; font-size:18px;">Confiance : <strong>{confiance}</strong></p>
</div>""",
					unsafe_allow_html=True,
				)
			else:
				st.markdown(
					f"""<div style="background-color:#f8d7da; border:2px solid #dc3545; border-radius:12px; padding:24px; text-align:center;">
<h2 style="color:#721c24;">❌ CRÉDIT REFUSÉ</h2>
<p style="color:#721c24; font-size:18px;">Confiance : <strong>{confiance}</strong></p>
</div>""",
					unsafe_allow_html=True,
				)
			...
			with c3:
				...
				st.metric("Emploi", type_emploi.replace("_", " ").title())
		else:
